...
def build_position_vocab(pos_lst, pos_vocab_file_name=None):
    # input: [tr_d1_pos_lst, te_d1_pos_lst]
    # d1, d2 각각 따로 함수 호출해야 함
    if os.path.exists(os.path.join(data_dir, pos_vocab_file_name)):
        wl = []
        with open(os.path.join(data_dir, pos_vocab_file_name), 'r', encoding='utf-8') as f:
            for line in f:
        ...
        # Append <PAD>
        wl.insert(0, '<PAD>')
        with open(os.path.join(data_dir, pos_vocab_file_name), 'w', encoding='utf-8') as f:
...
def load_data(unk_limit, max_sent_len, dev_size):
    ...
    d1_vocb = build_position_vocab([tr_d1_pos_lst, te_d1_pos_lst], pos_vocab_file_name='d1_vocab')
    d2_vocb = build_position_vocab([tr_d2_pos_lst, te_d2_pos_lst], pos_vocab_file_name='d2_vocab')
...
def load_test_data(unk_limit, max_sent_len):
    te_sentences, te_drug1_lst, te_drug2_lst, te_rel_lst, te_d1_pos_lst, te_d2_pos_lst, te_pos_tuple_lst = load_sentence(test_filename, max_sent_len)

    # Build distance position vocb
    d1_vocb = build_position_vocab(None, pos_vocab_file_name='d1_vocab')
    d2_vocb = build_position_vocab(None, pos_vocab_file_name='d2_vocab')
    ...
    # Build vocab only with train data
    vocb, vocb_inv = build_word_vocab(None)
    ...
    return (te_sentences2idx, te_d1_pos_lst, te_d2_pos_lst, te_pos_tuple_lst, te_y), (vocb, vocb_inv), (d1_vocb, d2_vocb), \
...
if __name__ == '__main__':
    # load_data(unk_limit=3000, max_sent_len=150, dev_size=0.1)
    load_test_data(unk_limit=3000, max_sent_len=150)
